


.. _Applied deck forces - RISA model:

**2.1.1** | Applied deck forces - RISA model
--------------------------------------------------------------------------------
 
 

.. figure:: c:/git/rivt-example-03-git/rivt-report/rvsrc/img/risa4.png
   :width: 70%
   :align: center

   **Fig. 1** - Risa Model   
    


 
 


--------------



.. raw:: pdf

   PageBreak



.. _Resultant axial forces - RISA model:

**2.1.2** | Resultant axial forces - RISA model
--------------------------------------------------------------------------------
 

.. figure:: c:/git/rivt-example-03-git/rivt-report/rvsrc/img/risa6.png
   :width: 70%
   :align: center

   **Fig. 2** - Strut Axial Forces   
    


 
 


--------------



.. raw:: pdf

   PageBreak



.. _Top rail shear reactions - RISA model:

**2.1.3** | Top rail shear reactions - RISA model
--------------------------------------------------------------------------------
 
Under the California Building Code (CBC), handrails and guards (railings)
must resist a uniform load of 50 plf and a concentrated point load of 200
lbs, both applied horizontally to the top rail. Intermediate rails,
balusters, and infill panels must separately withstand a concentrated load
of 50 lbs.
 


.. code-block:: text 



         =======  <-- Top Rail 
            |     <-- Lateral Load (P) 
            |  
            | 
            | 
            |  Height (h) 
            | 
            | 
            | 
            | 
       ===========  <-- Fixed Support / Deck Surface 
 
 
       Structural Schematic of Railing and Loads Drawn by AI 

 
 

.. figure:: c:/git/rivt-example-03-git/rivt-report/rvsrc/img/risa9.png
   :width: 60%
   :align: center

   **Fig. 3** - Rail Lateral Forces   
    


 
 
